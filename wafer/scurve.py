...
def shaped_s_curve_velocity(t, v_max, t_acc, t_const, t_dec):
    if t < t_acc:
        return v_max*(np.sin(np.pi*t/(2*t_acc)))**2
    elif t < t_acc+t_const:
        return v_max
...
def run_motor_scurve(gpio,encoder,motor_id,direction,total_steps,v_max,shape="mid"):
    ...
    if shape=="short": t_acc,t_dec,t_const=total_time*0.5,total_time*0.5,0
    ...
    moved_steps=0; prev_enc=encoder.read(); enc_init=(prev_enc/1000)*360
    start=time.time(); data_log=[]
    while moved_steps<total_steps:
        t=time.time()-start
        if t>total_time: break
        com_vel_steps=shaped_s_curve_velocity(t,v_eff,t_acc,t_const,t_dec)
        if com_vel_steps<1e-6: time.sleep(0.001); continue
        ...
        moved_steps+=1
        com_pos=moved_steps*DEG_PER_STEP; com_vel_deg=com_vel_steps*DEG_PER_STEP
        enc_now=encoder.read(); delta=enc_now-prev_enc; prev_enc=enc_now
        enc_pos=(enc_now/1000)*360-enc_init
        enc_vel=(delta/pulse_interval)*(360/1000) if delta!=0 else 0
        data_log.append([int(t*1000),com_pos,enc_pos,com_vel_deg,enc_vel])
    return data_log
...
def as_curve_velocity(t,v_max,t_acc,t_dec,total_time):
    t_const=total_time-t_acc-t_dec
    if t< t_acc:
        return v_max*(np.sin(np.pi*t/(2*t_acc)))**2
    elif t< t_acc+t_const:
