...
def _resample_to_fixed_ms(rows, step_ms=10):
    """rows:[[Time_ms, Com_Pos_deg, Pul_Pos_deg, Com_Vel_deg_s, Pul_Vel_deg_s]]
    를 0, step_ms, 2*step_ms,... 고정 간격으로 보간 없이 '홀드' 방식으로 리샘플링.
    마지막 시각까지 포함.
    """
    if not rows:
        return []
    ...
    max_t = int(rows_sorted[-1][0])
    out = []
    i = 0
    last = rows_sorted[0]
    for t in range(0, max_t + 1, step_ms):
        while i + 1 < len(rows_sorted) and rows_sorted[i + 1][0] <= t:
            i += 1
            last = rows_sorted[i]
        # 고정 간격 시각으로 교체
        out.append([t, last[1], last[2], last[3], last[4]])
    return out
...
def run_motor_scurve(
    ...
    *, log_stride: int = 10, high_time_min: float = DEFAULT_HIGH_TIME_MIN
    ...
    return _resample_to_fixed_ms(_raw, step_ms=10)
