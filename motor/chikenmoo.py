...
def move_stepper_scurve_with_logging(
    ...
    def sample_all(ts):
        nonlocal last_sample_t, last_sample_pulse_count
...
def main():
    ...
    try:
        ...
        while True:
            ...
            if pulse_rpm is not None and len(pulse_rpm):
                finite_mask = np.isfinite(pulse_rpm)
                if finite_mask.any():
                    rpm_values = pulse_rpm[finite_mask]
                    avg_rpm = float(np.mean(rpm_values))
                    max_rpm = float(np.max(rpm_values))
                    print(f"[INFO] Pulse-based RPM avg={avg_rpm:.2f}, max={max_rpm:.2f}")
                else:
                    print("[WARN] Pulse-based RPM contains only NaNs.")
