...
def move_stepper_scurve_with_logging(
    ...
    def sample_all(ts):
        ...
        if ts - last_sample_t >= enc_sample_dt:
            ...
            rpm, rad_s = pulses_to_speed(pulse_delta, dt, pulses_per_rev)
            pulse_rpm_log.append(rpm)
            pulse_rad_s_log.append(rad_s)
            deg_per_s = rpm * 6.0 if math.isfinite(rpm) else (rad_s * 180.0 / math.pi if math.isfinite(rad_s) else float("nan"))
            ...
            if math.isfinite(pulse_deg_per_s_lpf):
                ...
                pulse_rad_s_filtered_log.append(math.radians(pulse_deg_per_s_lpf))
            else:
                pulse_deg_per_s_filtered_log.append(float("nan"))
                pulse_rpm_filtered_log.append(float("nan"))
                pulse_rad_s_filtered_log.append(float("nan"))
    ...
    out = {"t": np.array(time_log, dtype=float),
           ...
           "pulse_rad_s": np.array(pulse_rad_s_log, dtype=float),
           ...
           "pulse_rad_s_filtered": np.array(pulse_rad_s_filtered_log, dtype=float)}
...
def main():
    ...
    try:
        ...
        while True:
            ...
            pulse_rad_s = logs.get('pulse_rad_s', None)
            pulse_rad_s_filtered = logs.get('pulse_rad_s_filtered', None)
            ...
            if has_pulse_rpm or has_pulse_rpm_f and not pulse_vel_plot:
                rpm_series = []
                rpm_labels = []
            ...
            has_pulse_rad = pulse_rad_s is not None and len(pulse_rad_s) == len(t)
            has_pulse_rad_f = pulse_rad_s_filtered is not None and len(pulse_rad_s_filtered) == len(t)
            if has_pulse_rad or has_pulse_rad_f:
                rad_series = []
                rad_labels = []
                if has_pulse_rad:
                    rad_series.append(pulse_rad_s)
                    rad_labels.append("pulse ω raw [rad/s]")
                if has_pulse_rad_f:
                    rad_series.append(pulse_rad_s_filtered)
                    rad_labels.append("pulse ω LPF [rad/s]")
                plot_time_series(t, rad_series, rad_labels, "Pulse-derived Angular Velocity (rad/s)", "Angular velocity (rad/s)", "pulse_speed_rad")
            ...
            if pulse_rad_s is not None and len(pulse_rad_s) == len(t):
                plot_time_series(t, [pulse_rad_s], ["pulse ω"], "Pulse-derived Angular Velocity", "Angular velocity (rad/s)", "pulse_speed_rad")
